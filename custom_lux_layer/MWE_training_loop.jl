...
function generate_polynomial(degree)
    ...
    #I1, J1 = @IJ degree
    #I = eval(I1)   # Did not work
    #J = eval(J1)
    #"""

    # The better way is to create a program to create the numerical indexes needed and inlude it in this file. 
    # In that way there will be no push! and mutations will no longer exist
    if degree == 2
    ...
    # Ideally, use @Tullio to generate these sequences
...
model = Polylayer(; out_dims=2, degree=2, init_weight=Lux.rand32)
...
@time main(; dct..., data=(x, y), epochs=5, dct[:ps].coeffs)
