...
function ude_dynamics!(du,u, p_NN, t, p_LV, U)
    û = U(u, p_NN) # Network prediction
    du[1] = p_LV[1]*u[1] + û[1]
    du[2] = -p_LV[4]*u[2] + û[2]
end

# Closure with the known parameter
nn_dynamics!(du,u,p,t) = ude_dynamics!(du,u,p,t,p_,U_)
# Define the problem
print("tspan: ", tspan)
...
function predict(θ, X = Xₙ[:,1], T = t)
    println("predict: T[1], T[end]: ", T[1], T[end])
    Array((solveprob_nn, Vern7, u0 = X, p=θ,
...
res1 = DiffEqFlux.sciml_train(loss, p_NN, ADAM(0.1f0), cb=callback, maxiters = 200)
